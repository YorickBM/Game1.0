...
import Inventory.InventoryMechanisme;
...
public class Player extends Entity {
    ...
    //Manage Items
    public void checkItem() {//Hard coded key controls, Need to remove entity from entities list afther pickedup
    ...
    public void dropItem(int slot) {
    	...
    	MainGameLoop.InvGui.remove(ItemTexture);
    	MainGameLoop.entities.add(new Entity(model, 1, loc, 0, 360, 0, 1f));
	...
	public void renderItem() { //hard coded int's
    	for(Entry<?, ?> e: map.entrySet()){    		
    		...
    		if(!Itemtexture.contains(e)) {
    			...
    			MainGameLoop.InvGui.add(ItemTexture);
    		}
    	}
    }
    ...
    private void checkInventory() { // Hard coded Controls
    	//Keyboard controls of Inventory
		if(Keyboard.isKeyDown(Keyboard.KEY_0)) this.activeSlot = 9;
		if(Keyboard.isKeyDown(Keyboard.KEY_1)) this.activeSlot = 0;
		if(Keyboard.isKeyDown(Keyboard.KEY_2)) this.activeSlot = 1;
		if(Keyboard.isKeyDown(Keyboard.KEY_3)) this.activeSlot = 2;
		if(Keyboard.isKeyDown(Keyboard.KEY_4)) this.activeSlot = 3;
		if(Keyboard.isKeyDown(Keyboard.KEY_5)) this.activeSlot = 4;
		if(Keyboard.isKeyDown(Keyboard.KEY_6)) this.activeSlot = 5;
		if(Keyboard.isKeyDown(Keyboard.KEY_7)) this.activeSlot = 6;
		if(Keyboard.isKeyDown(Keyboard.KEY_8)) this.activeSlot = 7;
		if(Keyboard.isKeyDown(Keyboard.KEY_9)) this.activeSlot = 8;
    ...
    private void checkInputs() { // < Hard coded controls
    ...
    public void setStats(String stat, int value) {
    	if(stat == "health") this.health = value;
    	if(stat == "mana") this.mana = value;
    	if(stat == "stamina") this.stamina = value;
    	if(stat == "souls") this.souls = value;
    	if(stat == "money") this.money = value;
    	if(stat == "xp") this.xp = value;
